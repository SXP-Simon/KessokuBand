...
class TrainingArguments(openmind.TrainingArguments):
    ...
    evaluation_strategy: str = field(default="steps", metadata={"help": "Evaluation strategy to adopt during training."})
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer):
        ...
        list_data_dict = jload(data_path)  # 加载 JSON 文件

        logging.warning("Formatting inputs...")
        prompt_input = PROMPT_DICT["prompt_input"]
        prompt_no_input = PROMPT_DICT["prompt_no_input"]
        prompt_choice = PROMPT_DICT["prompt_choice"]

        sources = []
        targets = []

        for example in list_data_dict:
            if "question" in example and "option" in example:
                # 选择题格式
                if len(example["option"]) != 4:
                    raise ValueError("Each example must have exactly 4 options.")
                source = prompt_choice.format_map({
                ...
                target = f"The correct answer is ({example['output']}).{tokenizer.eos_token}"
            else:
                # 原始格式
                if "input" in example:
                    source = prompt_input.format_map({
                        "instruction": example["instruction"],
                        "input": example["input"],
                        "output": example["output"]
                    })
                else:
                    source = prompt_no_input.format_map({
                        "instruction": example["instruction"],
                        "output": example["output"]
                    })
...
def train():
    # NOTE 替换 transformers.HfArgumentParser
    ...
    # Save PEFT weights (merged model)
